...
class dpd:
	def __init__(self, info, rcut=1.0, temperature=1.0):
		nl = info.find_plist(rcut, None)
		if nl is None:
			nl = plist.neighbor(info, rcut, None)
			info.plist.append(nl)
		self.info = info
		self.data=forces.pair.dpd(info, nl, temperature)
		self.name="force"
		self.subname="dpd"
		self.rcut = rcut
		self.check=[False for i in range(self.info.ntypes*self.info.ntypes)]
...
class ibidpd:
	def __init__(self, info, rcut=1.75, temperature=1.0, exclusion=None, fr_func=None, is_intra_frc_on=None, intra_neighbor_idx=[], intra_neighbor_size=[], A=1, alpha=1, persist_length=3, chain_length=10):
		nl = info.find_plist(rcut, exclusion)
		if nl is None:
			nl = plist.neighbor(info, rcut, exclusion)
			info.plist.append(nl)
		self.info = info
		self.data=forces.pair.ibidpd(info, nl, temperature, fr_func, is_intra_frc_on, intra_neighbor_idx, intra_neighbor_size, A, alpha, persist_length, chain_length)
		self.name="force"
		self.subname="ibidpd"
		self.rcut = rcut
		self.check=[False for i in range(self.info.ntypes*self.info.ntypes)]
	...
	def setParams(self, type_i, type_j, ibi_param, gamma):
		self.data.setParams(type_i, type_j, ibi_param, gamma, self.rcut, self.check)
...
class NonbondedTable:
	def __init__(self, info, rcut, exclusion=None):
		nl = info.find_plist(rcut, exclusion)
		if nl is None:
			nl = plist.neighbor(info, rcut, exclusion)
			info.plist.append(nl)
		self.info = info
		self.data=forces.pair.table_pair(info, nl)
		self.name="force"
		self.subname="nonbonded"
		self.check=[False for i in range(self.info.ntypes*self.info.ntypes)]
		self.first_compute = True
	...
	#def setParams(self, type_i, type_j, param):
	def setParams(self, type_i, type_j, filename, n_points, rcut):
		self.data.setParams(type_i, type_j, filename, n_points, rcut, self.check)
